...
- GroupNorm language encoders automatically use `torch.utils.checkpoint.checkpoint(..., use_reentrant=False)` for each residual-block/FiLM pair during gradient-enabled training. Recomputation retains the **same physical batch and FP32 math**, with no gradient accumulation or precision change. BatchNorm paths and inference do not recompute blocks, avoiding double running-statistic updates. No-language behavior is untouched. Full-batch memory/throughput qualification remains a separate run check.
...
Every policy has explicit categorical task conditioning: one-hot channels, ordered by sorted dataset task ID, are appended to the 25-D state. These remain literal 0/1 under normalization. Task maps are checkpoint-local; unseen IDs are rejected. The one-hot is categorical rather than language conditioning; optional `clip_film` adds a separate language input without removing it.
