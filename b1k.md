...
### Goal-image conditioning (`--regime`, `--goal-fusion`)

The `goal` branch (from `my@d149d0f` plus the language ports) adds goal-image conditioning to the two **hybrid image policies** (`transformer_hybrid_image`, the radio recipe's 12×512 Diffusion Transformer, and `unet_hybrid_image`) as configuration presets. Every run declares a **conditioning regime** — `--regime none` (N: observation only; on several tasks this is the only way to train without any task signal), `language` (L), `image` (I) or `image_language` (LI) — which fills defaults (L/LI: `clip_film`; I/LI: late fusion of the head goal) and rejects disagreeing `--language-conditioning` / `--goal-fusion` flags (`ModelConfig.validate` re-checks the saved configuration). `--no-task-onehot` (the CLI default on this branch) keeps the task id out of the state in all four conditions. Goal fusion requires global observation conditioning (inpainting detaches the encoder).

**Data contract.** `B1KLeRobotDataset(goal_views=..., goal_source=...)` builds a per-episode goal table once at construction (main process, pickled to the workers) and adds `goal_<camera>` to every sample as one `(3, S, S)` image with **no observation-history axis**; `--goal-source episode_last` (default) decodes the last frame of the episode's camera stream — through the frame cache when one is attached, so the goal path equals the camera path bit for bit — and `goal_key` the dataset's static `observation.goal_rgb.<camera>_camera_0` clip. The normalizer maps goal keys like their cameras ([0, 1] → [−1, 1]). `config.json` and every checkpoint carry a `conditioning` record (regime, language, goal fusion/views/encoder/source/sampling policy, task one-hot, denoiser, source commit, dataset).

**Encoder.** When language or goal conditioning is on, the policy uses `model/vision/clip_film.py::FiLMHybridObsEncoder`, our explicit re-implementation of robomimic's hybrid encoder (per camera ResNet18/GroupNorm → 32-keypoint SpatialSoftmax → Linear(64, 64) → ReLU; random crops in training, center crops in eval). `tests/test_b1k_goal.py` shows that with both switches off it is robomimic's encoder exactly (same parameter count, same outputs in eval and train mode), so I differs from N by the goal path only while the language-free base keeps robomimic's own module. `--goal-fusion early` channel-stacks each selected camera with its goal before the ResNet stem (`PairedConv2d`: one 6-channel stem `[W_obs, W_goal]` with the goal half zero-initialized, so the encoder equals the base at initialization and a zero goal is an exact absent goal; the pair is cropped jointly and the encoder output shape is unchanged — pure early fusion, no additional goal vector). `--goal-fusion late` encodes the goal once per sample with the camera's own encoder (`--goal-encoder shared_base`; `separate_base` is an identical copy, a capacity/sharing ablation) into a 64-D feature, cropped with the offsets of the current image of the **last** observation step it is paired with; goal encodings are computed outside the denoising loop and are not repeated over observation steps. With `clip_film`, the goal pass runs the FiLM layers at the identity unless `--language-on-goal-encoder` (saved as `language_on_goal_encoder`); language FiLM in the visual encoder is the only language site, no second language vector enters the denoiser.

**Denoiser interfaces.** Transformer: `TransformerForDiffusion(goal_cond_dim=64·views)` appends one goal token after the observation tokens with its own learned position (its role identity) and a **fully visible column** in the time-based `memory_mask` — a future image is a task condition, not an observed timestep, so every action position attends to it (the base rule `t ≥ s − 1` would have hidden it from the first actions); `goal_valid` masks an absent goal out of the cross-attention and of the condition encoder. U-Net: `c = concat(c_obs, goal_embedding)` widens the global condition projection. Noise/sampling schedule, prediction target, widths, horizon, action masking, causality and observation history are unchanged.

Claims: the late family is our controlled extension of the actual `my` denoiser; the LI compositions (RoboCasa365-style CLIP FiLM + either goal path) are compositions of published mechanisms, not a verified published joint baseline; the BPP Goal-Image (shared fine-tuned CLIP encoder + CNN U-Net) and Distill-D/MDT (frozen CLIP goal encoder) reference presets are **not implemented** here — they change the visual encoder/denoiser and belong to a separate reference family.

Two-task mixture and smoke recipe: `/tmp/dev/datasets/2026-challenge-demos-radio-navpickup-goal` (built by the ACT branch's `scripts/b1k/merge_lerobot_roots.py`; camera videos hard-linked from the challenge demos so the 96 px frame cache validates unchanged) and `scripts/b1k/run_navpickup_conditioning_smoke.sh` (`vanilla | none | language | image-early | image-late | image_language-early | image_language-late`, 5,000 steps, batch 8,960, the b1k preset; see b1k_runs.md). Serving: goal-conditioned checkpoints take the goal image under `goal::<camera observation key>` or a fixed `--goal-image CAMERA=PATH` file per view; the handshake reports `regime`, `task_onehot`, `goal_fusion`, `goal_views`, `goal_observation_keys` and `goal_source`.

CPU coverage: `tests/test_b1k_goal.py` (11 cases) — goal table = episode last frame (native and cached) and dedicated stream; robomimic equivalence; paired-stem initialization, absent-goal and 6-channel equivalences; goal token visibility, masking (decoder and condition encoder) and construction errors; once-per-sample goal encoding and goal sensitivity for both hybrid policies; identity FiLM on the goal pass; configuration/regime validation; bit-exact train/resume/serve lifecycle with task-id invariance, goal sensitivity and fixed goals. Full suite on 2026-09-20: 282 passed, 1 skipped.
